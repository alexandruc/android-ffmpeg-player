...
jint naInit(JNIEnv *pEnv, jobject pObj, jstring pFileName) {
	...
	if(avformat_open_input(&formatCtx, videoFileName, NULL, NULL)!=0){
		LOGE("FAILED to open input %s", videoFileName);
		return -1; // Couldn't open file
	}
	// Retrieve stream information
	if(avformat_find_stream_info(formatCtx, NULL)<0){
		LOGE("FAILED to find stream info %s", videoFileName);
...
jintArray naGetVideoRes(JNIEnv *pEnv, jobject pObj) {
	...
	LOGI("width: %d, height: %d", codecCtx->width, codecCtx->height);
	jint lVideoRes[2];
...
jint JNI_OnLoad(JavaVM* pVm, void* reserved) {
	...
	jclass cls = (*env)->FindClass(env, "com/example/android_ffmpeg_player/MainActivity");
